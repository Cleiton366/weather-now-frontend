...
export default function ForecastPanel() {
  ...
  function setSwitchStyle(forecastOption: number) {
    if (forecastOption === 7) {
      return forecast === 7 ? 'flex bg-[#34376d] rounded-[15px] w-20 h-10 items-center justify-center mr-1 text-white md:text-[12pt] border rounded-[25px] border-[#42434e]' : 'md:text-[12pt] mx-3';
      ...
      <div className="flex flex-col items-center md:flex-row justify-between">
        ...
        >
          <div className="flex justify-between items-center">
            ...
            <span className={`${setSwitchStyle(7)} text-[10pt]`}>
              7 days
            </span>
          </div>
        </div>
            ...
            <div key={i} className="flex justify-between items-center md:p-2 px-2 md:px-10 bg-[#34376d] rounded-[1.2rem]">
              <div className="flex items-center">
                ...
                <span className="font-bold text-[14pt] md:text-[20pt] ml-5">{data.temperature}°
                  <span className="font-bold text-[8pt] md:text-[10pt]"> /{data.min}°</span>
                </span>
              </div>
              <h1 className="font-bold text-[14pt] md:text-[20pt]">{data.date.slice(0, 2)}
                <span className="font-bold text-[8pt] md:text-[10pt] ml-2">{data.date.slice(2, data.date.length)}</span>
              </h1>
            </div>
          ))
        }
